...
if (isset($_POST["add_to_cart"])) {
  if (isset($_SESSION["shopping_cart"])) {
    ...
    if(!in_array($_GET["id"], $item_array_id)){
      ...
      echo '<script>window.location="NikeMenu.php"</script>';
    }
  }else {
    $item_array = array(
      'item_id'       => $_GET["id"],
...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Tondo Shoe Mates/Nike</title>
...
<div class="row">
  ...
  if(mysqli_num_rows($result) > 0){
    while($row = mysqli_fetch_array($result)){
      ?>
      <div class="col-md-4">
        <form  class="prod_display" method="post" action="NikeMenu.php?action=add&id= <?php echo $row["id"]; ?> ">
        ...
        if (!empty($_SESSION["shopping_cart"])) {
          ...
          foreach($_SESSION["shopping_cart"] as $keys => $values){
            ...
            <tr>
              ...
              <td> <a href="NikeMenu.php?action=delete&id=<?php echo $values["item_id"]; ?>"> <span class="text-danger">Remove</span> </a> </td>
            </tr>
            <?php
